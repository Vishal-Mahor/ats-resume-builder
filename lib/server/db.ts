...
async function ensureOptionalColumns(currentClient: Client) {
  const migrationStatements = [
    ...
    `ALTER TABLE resumes ADD COLUMN sort_order INTEGER`,
    `ALTER TABLE resumes ADD COLUMN base_resume_id TEXT REFERENCES resumes(id) ON DELETE SET NULL`,
    `ALTER TABLE resumes ADD COLUMN job_description TEXT`,
    `ALTER TABLE resumes ADD COLUMN job_url TEXT`,
    `ALTER TABLE job_applications ADD COLUMN sort_order INTEGER`,
